            ...
            <div>
                {this.props.list.results.map(item => {
                    ...
                    <Card elevation={5} key={item.trackId} className={classes.card}>
                        ...
                        {/*</CardActionArea>*/}
                        <CardActions >
                            <IconButton onClick={this.toggleFavorite(item.trackId, item.collectionName, item.artistName, item.primaryGenreName, item.trackCensoredName, item.artworkUrl100)}>
                            {this.state.likes[item.trackId] ? <Favorite /> : <FavoriteBorder />}
                            </IconButton>
                        </CardActions>
                    </Card>)
                })}
